          ...
          return (
            <button key={item.id} type="button" onClick={() => { haptic.light(); onSelectModule(item.id); }} aria-label={isFa ? item.label.fa : item.label.en} aria-current={isActive ? 'page' : undefined} className={`flex-1 h-11 px-1 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all duration-200 cursor-pointer relative active:scale-95 border ${isActive ? `${item.activeBg} font-black` : 'text-slate-400 hover:text-slate-200 border-transparent hover:bg-black/5 dark:hover:bg-white/5 opacity-80 hover:opacity-100'}`}>
              ...
              <div className="relative">
                <Icon className={`w-[19px] h-[19px] transition-all duration-200 ${isActive ? `${item.activeColor} scale-105 stroke-[2.4]` : 'stroke-[1.8]'}`} />
                {typeof item.badge === 'number' && item.badge > 0 && <span className="absolute -top-1.5 -end-2.5 px-1 min-w-3.5 h-3.5 rounded-full bg-linear-to-r from-rose-500 to-pink-500 text-white font-mono text-[9px] font-black flex items-center justify-center shadow-xs ring-1 ring-black/20 animate-pulse">{item.badge > 99 ? '99+' : item.badge}</span>}
              </div>
              <span className={`text-[9.5px] sm:text-[10px] tracking-tight leading-none truncate ${isActive ? 'font-black' : 'font-medium'}`}>{isFa ? item.label.fa : item.label.en}</span>
            </button>
          );
        })}
      </div>
