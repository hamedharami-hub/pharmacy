  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3">
      <div
        ...
        className={`app-card border app-border rounded-2xl p-3 sm:p-3.5 flex items-center justify-between transition ${
          onOpenAnalytics ? 'cursor-pointer hover:border-emerald-500/50 hover:bg-emerald-500/5 group' : ''
        }`}
      ...
      >
        <div>
          <span className="text-[10.5px] sm:text-[11px] font-bold app-muted block flex items-center gap-1">
            ...
            {onOpenAnalytics && (
              <TrendingUp className="w-3 h-3 text-emerald-400 opacity-70 group-hover:opacity-100 transition" />
            )}
          </span>
      ...
      <div className="app-card border app-border rounded-2xl p-3 sm:p-3.5 flex items-center justify-between">
        <div>
          <span className="text-[10.5px] sm:text-[11px] font-bold app-muted block">
            {isFa ? 'کل مباحث' : 'Total Topics'}
          </span>
          <span className="text-base sm:text-lg font-extrabold app-text font-mono">
            {totalCards}
          </span>
        </div>
      ...
      <div
        onClick={onOpenAnalytics}
        className={`app-card border app-border rounded-2xl p-3 sm:p-3.5 flex items-center justify-between transition ${
          onOpenAnalytics ? 'cursor-pointer hover:border-purple-500/50 hover:bg-purple-500/5 group' : ''
        }`}
      ...
      >
        <div>
          <span className="text-[10.5px] sm:text-[11px] font-bold app-muted block flex items-center gap-1">
            ...
            {onOpenAnalytics && (
              <TrendingUp className="w-3 h-3 text-purple-400 opacity-70 group-hover:opacity-100 transition" />
            )}
          </span>
          <span className="text-base sm:text-lg font-extrabold text-purple-400 font-mono">
            {quizScorePct}%
          </span>
        </div>
