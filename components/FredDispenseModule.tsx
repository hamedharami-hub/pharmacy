          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 text-[11px]">
            ...
            <button
              ...
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                viewMode === 'terminal' || viewMode === 'dual' ? 'bg-teal-950/60 border-teal-500/60 text-teal-200' : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-teal-400 font-bold uppercase">Step 2</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۲. ورود به فرِد' : '2. Terminal'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'کد PBS و جایگزینی' : 'PBS & Brand'}</span>
            </button>

            <button
              onClick={() => setViewMode('shortcuts')}
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                viewMode === 'shortcuts' || viewMode === 'dual' ? 'bg-amber-950/60 border-amber-500/60 text-amber-200' : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-amber-400 font-bold uppercase">Step 3</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۳. میانبرها' : '3. Shortcuts'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'قوانین 5/1، 5D، 5R' : 'Syntax & 5/1'}</span>
            </button>

            <button
              onClick={() => setViewMode('safetynet')}
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                viewMode === 'safetynet' || viewMode === 'dual' ? 'bg-emerald-950/60 border-emerald-500/60 text-emerald-200' : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-emerald-400 font-bold uppercase">Step 4</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۴. Safety Net' : '4. Safety Net'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'محاسبه فرانشیز' : 'Co-pay & Rules'}</span>
            </button>

            <button
              onClick={() => setViewMode('labelingDesk')}
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                viewMode === 'labelingDesk' || viewMode === 'dual' ? 'bg-teal-950/60 border-teal-500/60 text-teal-200' : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-teal-400 font-bold uppercase">Step 5</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۵. برچسب‌گذاری' : '5. Labeling'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'لیبل و استیکر PBS' : 'CAL & Audit Sticker'}</span>
            </button>

            <button
              onClick={() => setViewMode('retentionDesk')}
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                viewMode === 'retentionDesk' || viewMode === 'dual' ? 'bg-indigo-950/60 border-indigo-500/60 text-indigo-200' : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-indigo-400 font-bold uppercase">Step 6</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۶. بایگانی اسناد' : '6. Retention'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'منگنه PB24 و زونکن' : 'PB24 Stapling'}</span>
            </button>

            <button
              onClick={() => setViewMode('odtDosing')}
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                scenario.schedule === 'S8' || scenario.scriptType === 'typeE'
                  ? 'bg-rose-950/70 border-rose-500/70 text-rose-200 ring-1 ring-rose-500/50'
                  : viewMode === 'odtDosing' || viewMode === 'dual'
                  ? 'bg-rose-950/40 border-rose-500/40 text-rose-300'
                  : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-rose-400 font-bold uppercase">Step 7</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۷. ثبت دوز ODT' : '7. ODT Log'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'دفترچه S8 و دوز روزانه' : 'S8 Register Safe'}</span>
            </button>

            <button
              onClick={() => setViewMode('pbsArchive')}
              className={`p-2 rounded-xl border text-right transition flex flex-col justify-between h-full ${
                viewMode === 'pbsArchive' || viewMode === 'dual' ? 'bg-purple-950/60 border-purple-500/60 text-purple-200' : 'bg-slate-950/40 border-slate-800 text-slate-400'
              }`}
            >
              <span className="font-mono text-[9px] text-purple-400 font-bold uppercase">Step 8</span>
              <span className="font-bold text-xs truncate mt-0.5">{isFa ? '۸. ادعای PBS' : '8. PBS & POS'}</span>
              <span className="text-[9px] opacity-70 truncate">{isFa ? 'امحای PII و تحویل POS' : 'POS Release'}</span>
            </button>
          </div>
        </div>
